...
def process_chunk(rec, message):
    if isinstance(message, (bytes, bytearray)) and message == b'{"eof":1}':
        return rec.FinalResult(), True
    elif isinstance(message, str) and message.strip() == '{"eof":1}':
        return rec.FinalResult(), True
...
async def recognize(ws):
    ...
    args.show_words = os.environ.get('VOSK_SHOW_WORDS', 'True').lower() in ('1','true','yes')
    ...
    while True:
        ...
        if stop:
            logging.info("EOF recibido, cerrando conexión")
            break
